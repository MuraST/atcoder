...
struct Sum2D {
    ...
    std::array<std::array<T, W + 1>, H + 1> csum;

    constexpr Sum2D() noexcept : init{}, data{}, csum{} {}
    ...
    [[nodiscard]] T get(const int y, const int x) const noexcept {
        assert(0 <= y && y < H);
        assert(0 <= x && x < W);
    ...
    void set(const int y, const int x, const T v) noexcept {
        assert(0 <= y && y < H);
        assert(0 <= x && x < W);
    ...
    [[nodiscard]] T sum(const int sy, const int sx,
                        const int ty, const int tx) const noexcept {
        assert(init);
        assert(0 <= sy && sy < H);
        assert(0 <= sx && sx < W);
        assert(0 <= ty && ty <= H);
